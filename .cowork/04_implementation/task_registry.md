...
| TASK-041 | CMakePresets.json 지원 (Preset 칩) | MS-012 | INT-001 | US-003·US-012 | AI | Planned | 2026-08-16 | 신규(세션 #009). `CMakePresets.json`(+User) `configurePresets` 읽어 **Preset 칩** → `cmake --preset <name>` configure(파일 무편집·읽기전용, ADR-013). 프리셋 있으면 profile/architecture 칩 대체(컴파일러+제너레이터+빌드타입 인코딩)·없으면 현행 `-S -B -D` 폴백. target 칩·디버거 자동판별 재사용. 파싱: `inherits`+`${sourceDir}` 매크로(v1 흔한 것만). **F5** → MS-012 Done. 실 프로젝트 GCC/Clang/MSVC 전환 충족 | TASK-035 | | C-7 3/3 |
...
> 현재 등록 Task: TASK-001~041. **TASK-001~035 Done** (MS-001~011 + MS-012 TASK-033·034·035). **MS-012 CMake 핵심 루프(switch/build/run/debug) 완성**(F5 통과·커밋, 브랜치 `feature/ms-012-cmake-adapter` 미병합). 다음 활성: **TASK-041**(CMakePresets.json — 실 프로젝트 GCC/Clang/MSVC 전환) → MS-012 F5 → main 병합. 이후 **MS-013 Run Group**(TASK-036~040, C-6). **C-3 폐기**(D-15). 별도 트랙: TC-11(WSL, Deferred). ※ CMakePresets는 037~040이 MS-013 예약이라 **TASK-041**로 채번(세션 #009 정정).
