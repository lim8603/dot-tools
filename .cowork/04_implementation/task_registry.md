...
| TASK-033 | CMakeBridge + listProjects + chips (+연동 ADR) | MS-012 | INT-001 | US-003 | AI | In Progress | 2026-08-16 | **ADR-014 채택**(자체 `cmake` CLI 구동·File API, CMake Tools 미위임). **1차 슬라이스 완료**: `cmakeBridge.ts`(checkToolchain·parseCMakeVersion·cmakeProjectName·exec 주입) + `cmakeAdapter.collectDiagnostics`(cmake critical) + invalidateCache 실구현. unit +7(총 **142**). **Doctor 실테스트 가능**(cmake 미설치 → ❌+E1). **남음**: listProjects(`**/CMakeLists.txt`) + chips(profile=CMAKE_BUILD_TYPE 정적·target=File API) | 없음 | | C-7 3/3 |
...
> 현재 등록 Task: TASK-001~040. **TASK-001~029 Done** (MS-001~010). **MS-011 코드 완료**: TASK-030(감지+칩)·TASK-031(실행)·TASK-032(디버그+진단) 전부 Review — **F5 게이트 대기**. 다음 활성: **MS-011 F5**(Python 등장·빌드버튼 미표시·env/target 칩·Run·debugpy 중단점·Doctor) → 통과 시 MS-011 Done → MS-012 CMake(033~035, 착수 시 CMake Tools 연동 ADR)·MS-013 Run Group(036~040). **C-3은 폐기**(D-15). 별도 트랙: 수동검증 TC-11(WSL, Deferred).
