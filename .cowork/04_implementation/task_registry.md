...
| TASK-030 | PythonBridge + listProjects + chips | MS-011 | INT-001 | US-003 | AI | Review | 2026-08-16 | **코드 완료·F5 대기** — `pythonBridge.ts`(detectVersion 캐시·checkToolchain·DI + 순수 helper pythonProjectName/venvInterpreter/assemblePythonArgs) + dotnetAdapter listProjects(`**/pyproject.toml`, venv/cache 제외, 이름=폴더명) + chips(environment=venv+시스템 자체탐지·target=.py 나열 기본 main.py, required) + invalidateCache + 카탈로그 env 라벨 정정(PYTHONDONTWRITEBYTECODE). **실 python 3.12 스모크 통과**. unit +5(총 128)·esbuild OK. F5: Python 프로젝트 등장·**빌드버튼 미표시(리트머스)**·env/target 칩 | 없음 | | C-7 2/3 리트머스 |
...
> 현재 등록 Task: TASK-001~040. **TASK-001~029 Done** (MS-001~010: L-1·persistSetting 제거·**C# 어댑터 실구현 F5 통과**). 다음 활성: **TASK-030(PythonBridge, MS-011 C-7 2/3)**. 남은 것: MS-011 Python(030~032)·MS-012 CMake(033~035)·MS-013 Run Group(036~040). **C-3은 폐기**(D-15). 별도 트랙: 수동검증 TC-11(WSL, Deferred).
