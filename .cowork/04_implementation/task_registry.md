...
| TASK-039 | (선택) 준비 감지 (포트/헬스체크) | MS-013 | INT-001 | 없음 | AI | Planned | 2026-08-17 | 준비 감지 = 포트/헬스체크(docker-compose `depends_on` 유사). **ADR-015에서 v1.0.0 범위 밖·후속 마이너로 분리 확정** — 멤버별 준비 조건(포트·URL·타임아웃) 옵션 | TASK-037 | | C-6 옵션(후속) |
...
| TASK-042 | 추가 기능·논의 + v1.0.0 완주 결정 (게이트) | MS-014 | INT-001 | 없음 | Human | Planned | 2026-08-17 | **MS-013(v0.4.0) 이후 게이트.** 남은 추가 기능 후보·논의 수집 → **v1.0.0 완주(INT-001 종료 처리) 여부를 Human이 명시적으로 결정**(D-19). 결정 전까지 v0.x.x 유지 | TASK-040 | | 신규(세션 #011, Human 지시). D-19 |

> 현재 등록 Task: TASK-001~042. **TASK-001~041 Done**(041=TASK-039 제외 전부). **MS-013 Run Group(C-6) 완료·v0.4.0 배포**(세션 #011, 036·037·038·040 Done, unit **203**, 태그 `v0.4.0` push). 스테이지 순서·skip·통합 메뉴(D-20). **TASK-039(준비감지=포트/헬스체크)는 후속 마이너로 분리**(Planned 유지). **다음: TASK-042**=추가 기능·논의 + **v1.0.0 완주 결정 게이트**(D-19, Human). **C-3 폐기**(D-15). 별도 트랙: TC-11(WSL, Deferred).
