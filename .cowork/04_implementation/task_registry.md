...
| TASK-036 | Run Group 상태 모델 + 정의 스키마 + 저장 | MS-013 | INT-001 | 없음 | AI | Planned | 2026-08-16 | 그룹 상태 모델 + 그룹 정의 스키마(멤버·종속 순서) + workspaceState 저장. **그룹 모델 ADR** 채번 | 없음 | | C-6 (완주=v1.0.0) |
...
| TASK-040 | Run Group 통합 테스트 + 검증 + 문서 | MS-013 | INT-001 | 없음 | AI | Planned | 2026-08-16 | 통합 테스트 + test_case/verification + README/CHANGELOG. **F5 검증** → MS-013 Done, **v1.0.0**. **INT-001 완주** | TASK-038 | | C-6 |

> 현재 등록 Task: TASK-001~041. **TASK-001~035 Done** (MS-001~011 + MS-012 TASK-033·034·035). **TASK-041 Review**(CMakePresets.json 코드 완료·실 cmake 스모크 통과·F5 대기, 세션 #010). MS-012 CMake는 핵심 루프(switch/build/run/debug) 완성 + 프리셋까지 구현 완료, F5 통과 시 Done·병합. 브랜치 `feature/ms-012-cmake-adapter`(미병합). 이후 **MS-013 Run Group**(TASK-036~040, C-6). **C-3 폐기**(D-15). 별도 트랙: TC-11(WSL, Deferred). ※ CMakePresets는 037~040이 MS-013 예약이라 **TASK-041**로 채번(세션 #009 정정).
