...
| TASK-006 | CargoAdapter 실구현 | MS-003 | INT-001 | US-003 | AI | Review | 2026-08-15 | 완료(M2 스코프) — listProjects·chips(profile/arch/features/target, format·defaultValue)·createBuild/RunTask(ProcessExecution, env/CARGO_TARGET_DIR)·resolveExecutable(build json+pickExecutable)·invalidateCache 실구현. peekMetadata 추가. tsc·eslint·esbuild·34테스트 OK. **의도적 이월**: createDebugConfig→M4(MS-005), createProjectTask→MS-008(F20), persistSetting→v2 | TASK-005 | | M2. cargo 실구현 (v1 타깃). 커스텀 profile·F19 target 자동설치·compiler/linker 오버레이는 후속 |
