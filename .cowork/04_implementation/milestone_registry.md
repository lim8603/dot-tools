...
| MS-010 | C# (Dotnet) 어댑터 실구현 | INT-001 | Build | In Progress | TASK-027~029 | | **코드 완료·MS-010 F5 대기**(TASK-027 감지·028 build/run/resolveExecutable·029 debug/진단, 전부 실 dotnet 10 스모크 통과). 메타데이터=`msbuild -getProperty`(JSON)·실행경로=TargetPath·주입=`-p:`·디버그=coreclr(C# Dev Kit). unit 123. C-7 1/3, v1.1. F5 통과 시 Done |
