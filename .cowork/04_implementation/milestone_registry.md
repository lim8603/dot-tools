...
| MS-016 | Node/TS 어댑터 실구현 (v0.6.0) | INT-001 | Build | In Progress | TASK-046(Done)·047·048 | | **로드맵 v0.6.0(D-21).** Node/TypeScript 어댑터 — 6번째 언어. 감지(`package.json`). **설계 확정(세션 #013·Human)**: ① 칩=**script + packageManager**(npm/pnpm/yarn) ② Build 버튼 有(`<pm> run build`, actions.build:true) ③ 디버그=**선택 npm 스크립트**(js-debug `runtimeExecutable`, 확장 불요). TASK-046 코드 완료(감지·2칩·createProject·Doctor, unit 227). **완료 시 6개 언어 완성.** |
