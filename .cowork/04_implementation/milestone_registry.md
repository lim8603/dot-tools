...
| MS-017 | 키보드 단축키 설정 (v0.7.0) | INT-001 | Build | In Progress | TASK-049·050 | | **로드맵 v0.7.0(D-21).** 설정 페이지 General 탭 기본 단축키 + 변경. **구현방식 확정(세션 #013·Human) → ADR-017**: 정적 `contributes.keybindings`(Ctrl+Alt+B/R/D/P/,·`when:hasProjects`) + 네이티브 편집기 딥링크(런타임 재바인딩 API 부재). **내장 키(F5/Ctrl+Shift+B) 불간섭 + 리맵 안내만**(Human). 팀 필수 UX |
