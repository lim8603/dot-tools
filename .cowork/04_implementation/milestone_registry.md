...
| MS-012 | C++ (CMake) 어댑터 실구현 | INT-001 | Build | Review | TASK-033~035·041 | 2026-08-17 | **ADR-014 채택**(자체 `cmake` CLI·File API). **전 Task F5 통과**: 033(감지+chips+File API)·034(prepareInvocation+configure/build `-D` 주입+resolveExecutable)·035(run build-then-exec+debug 컴파일러 자동판별 cppvsdbg/cppdbg+gdb/lldb+override)·**041(CMakePresets: `ChipDescriptor.appliesTo` 동적 대체 Preset 칩·`cmake --preset`·D-17)**. unit 172·실 cmake 4.4.2+VS18 스모크·실 F5(build/run/debug 중단점). **main 병합 시 Done.** C-7 3/3. v1.3 |
