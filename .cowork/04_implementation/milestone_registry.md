...
| MS-014 | v1.0.0 완주 — 최종 점검 + 게시 | INT-001 | Deliver | In Progress | TASK-042·054·055 | | **로드맵 최종 단계(D-21, 실행 순서상 MS-015~018 이후).** 통합 테스트 최종 보강(dotnet/cmake/python/go/node 픽스처) + **VS Code Marketplace 게시** + **GitHub Release 생성**(`v1.0.0` 태그 + vsix 첨부) → **v1.0.0 완주 선언**(INT-001 종료, Human). 이전엔 "완주 결정 게이트"였으나 D-21로 로드맵이 구체화되며 **v1.0.0 릴리즈 마일스톤으로 재정의** |
