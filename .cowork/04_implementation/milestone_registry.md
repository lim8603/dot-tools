...
| MS-013 | Run Group (C-6) | INT-001 | Build | In Progress | TASK-036~040 | | **착수(세션 #011).** 그룹 상태 모델+GroupOrchestrator(계층적 위상정렬·병렬/순차·teardown)+정의/트리거 UI. **ADR-015**: 준비=프로세스 시작(`onDidStartTaskProcess`)·Run 전용·헬스체크(TASK-039)는 후속 마이너 분리. TaskRunner 프로젝트별 락 재사용. **TASK-036 Review**(모델·저장·순수 계획/검증, unit 186). **완주=v0.4.0 배포**(C-6 충족) — v1.0.0 아님(D-19) |
| MS-014 | 추가 기능·논의 및 v1.0.0 완주 결정 (게이트) | INT-001 | Evolve | Planned | TASK-042 | | **MS-013(v0.4.0) 이후 게이트(세션 #011, Human 지시).** 남은 추가 기능 후보·논의 수집 → **v1.0.0 완주(INT-001 종료) 여부를 Human이 명시적으로 결정**(D-19). MS-013이 INT-001 등록 조건(C-7+C-6)을 충족해도 v1.0.0은 자동 트리거되지 않음 |

> 현재 등록 Milestone: MS-001~MS-014 (M0~M6 + F20 완료 / MS-009~013 = INT-001 완주 로드맵: 정리 + 다언어 C-7 + Run Group C-6 / MS-014 = v1.0.0 완주 결정 게이트)
...
- 의존 순서: MS-001 → … → MS-008 **전부 Done(2026-08-16, v0.2.0)**. **INT-001 완주 로드맵**: MS-009(정리) → MS-010 C# → MS-011 Python → MS-012 CMake(= C-7, **Done·v0.3.0**) → MS-013 Run Group(C-6, **In Progress → v0.4.0**) → **MS-014 v1.0.0 완주 결정 게이트**(D-19). **C-3(캐노니컬 파일 편집)은 폐기**(D-15 — "파일 무편집" 영구 불변식).
- **버전 정책(SemVer, D-19 갱신 2026-08-17)**: 1.0 이전은 `0.y.z` 개발 단계. 현재 배포=**v0.3.0**(C-7 3언어). **C-6 Run Group(MS-013) = 다음 릴리즈 `v0.4.0`**(신기능 MINOR). **v1.0.0은 INT-001 등록 조건(C-7+C-6) 충족만으로 자동 트리거되지 않는다** — 남은 추가 기능·논의(MS-014/TASK-042) 후 **Human이 명시적으로 완주 선언할 때** v1.0.0으로 올린다(D-19). 특별 지시 전까지 **v0.x.x 유지**. SemVer: 0.3.0 → 0.4.0(C-6) → (추가 기능 MINOR…) → 1.0.0(Human 선언). ※ 과거 "INT-001 완주=v1.0.0"·"v1.1/v1.2/v1.3/v2.0"은 폐기된 표기.
