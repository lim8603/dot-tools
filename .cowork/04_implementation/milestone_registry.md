...
| MS-010 | C# (Dotnet) 어댑터 실구현 | INT-001 | Build | In Progress | TASK-027~029 | | 스위치·빌드·실행·디버그·진단 실구현(현재 F20 생성만). C-7 1/3, v1.1. **착수 도구검증(2026-08-16)**: 메타데이터=`dotnet msbuild -getProperty`(JSON), 실행경로=`TargetPath`, 디버그=coreclr(C# Dev Kit). cargoBridge 패턴 준용 |
