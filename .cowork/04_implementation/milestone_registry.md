...
| MS-022 | v1.2.0 — Visual Studio 어댑터 + 언어별 enable(B-3) + 아이콘 리프레시 | 없음(유지보수) | Build | **Done** | TASK-063~065 | 2026-08-19 | **완료·v1.2.0 게시(세션 #017, EV-023/024, Gate 5 Pass).**  **Human 지시(세션 #017, D-25).** ① `vs` 어댑터(ADR-021): .vcxproj 감지(CMakeCache 마커 제외·A안 .csproj=dotnet 소유)·.sln 계층·MSBuild 빌드·TargetPath 실행·cppvsdbg 디버그 ② **B-3 해소**: `devSwitcher.languages.enabled` 스캔/표시 언어 필터 + General 탭 체크박스 ③ 아이콘 플랫 벡터 디자인 교체(Human 제작). v1.2.0 단일 릴리즈(F5 통과 후 스탬프·게시) |

> 현재 등록 Milestone: MS-001~MS-022 (M0~M6 + F20 완료 / MS-009~013 = INT-001 등록조건 로드맵 / **MS-015~018 + MS-014 = INT-001 v1.0.0 완주 로드맵(D-21)**: Go → Node/TS → 단축키 → 준비감지 → 최종점검+게시 / **MS-019·020 = INT-002**(원격디버그·크로스컴파일, v1.0.0 이후, D-22) / **MS-021 = 유지보수 v1.1.0**(실사용 피드백, ADR-019))
