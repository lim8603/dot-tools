...
- **MS-001~007 전부 완료 — v0.1.0 vsix(`devswitcher-tools-0.1.0.vsix`) 산출.** MS-007(M6)=Doctor·E1 칩·rustup target 자동설치·pre/postBuild·통합테스트 하네스 + **README·LICENSE·CHANGELOG·`.vscodeignore` + 상태바 목업 2종(실 codicon) + vsce 패키징·설치 스모크(TASK-021)**. mocha 92 + 통합 3. **상태바 전환·빌드·실행·디버그·설정 페이지·프로파일 export/import·Doctor 전부 실동작.** Gate 5 = README·VSIX 해소·설치 스모크 Pass로 v0.1 확정, 잔여 수동검증(TC-11 WSL 등)은 문서화된 리스크. **다음: MS-008(F20 시작 마법사) 착수 — Task 분해부터.**
