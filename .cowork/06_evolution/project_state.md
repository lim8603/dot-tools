...
| 활성 Task | TASK-033 (Doctor + listProjects + chips + File API **코드 완료, Review·F5 대기**) |
| 상태 | Green |
| 대화 언어 | 한국어 |
| 작업 문서 언어 | 한국어 |
| 공식 산출물 문서 언어 | 한국어 |
...
- **MS-001~011 Done** (MS-009·010·011 main FF 병합·origin push 완료). INT-001 완주 로드맵 = C-7(다언어 실구현)+C-6(Run Group). **현재: MS-012 CMake(C-7 3/3) 진행** — ADR-014(자체 `cmake` CLI 구동). **TASK-033 코드 완료(Review·F5 대기)**: Doctor + listProjects + chips(profile/architecture/target) + **CMake File API 계층**. 실 cmake end-to-end 스모크 통과(구성별 artifact 경로 해석). **C-3 폐기**(D-15).

### 현재 작업 스트림
> 핵심 작업 스트림만 3~5줄 이내로 유지한다.

- **완료**: MS-010 C#·MS-011 Python 병합·push. **TASK-033 코드 완료(F5 대기)** — CMakeBridge File API + listProjects + chips 3종. unit 154·esbuild 76.9kb. 실 cmake 스모크 통과.
- **다음**: TASK-033 F5(Human, chips 확인) + 커밋 → **TASK-034**(configure/build `-D` 주입+resolveExecutable=File API artifact 재사용) → 035(디버그+통합) → MS-012 F5·병합. 이후 C-6 Run Group(MS-013).
...
|---------|------|------|------|---------------|-----------|
| TASK-033 | CMakeBridge + listProjects + chips | AI | Review | 2026-08-16 | **코드 완료·F5 대기** — Doctor + `listProjects`(project() 루트 판별·이름) + chips 3종(profile 정적 4 build type·architecture 정적 플랫폼·**target=File API codemodel-v2**) + File API 계층(`readReplyDir`·`listTargets`). unit 154·실 cmake end-to-end 스모크 통과(Debug/Release artifact 경로 정확). 브랜치 `feature/ms-012-cmake-adapter`(미커밋). **다음**: F5(chips) + 커밋 → TASK-034 |
...
1. **⚠️ TASK-033 F5(Human)** — VS Code **완전 재시작**(cmake PATH 반영) 후 F5: ① Doctor에 **cmake 4.4.2 ✅** + E1 칩 소멸 ② `fixtures/cmake/hello`(또는 실 CMake 프로젝트)에서 **chips 확인**(profile 4종·architecture 플랫폼·**target=File API 실 타깃 등장**). configure는 `build/` 생성(gitignore됨).
2. **커밋(feat+docs) 후 TASK-034**(브랜치 `feature/ms-012-cmake-adapter`) — configure/build 태스크 `-D` 오버레이 주입(compiler CMAKE_CXX_FLAGS·linker·output buildDir·CMAKE_BUILD_TYPE·`-A`) + resolveExecutable(**File API artifact 재사용** — 스모크로 경로해석 검증됨) → TASK-035(디버그+디버거 확장 확정+통합) → MS-012 F5·병합.
...
| Milestone | MS-012 | C++ (CMake) 어댑터 실구현 | In Progress | C-7 3/3. ADR-014(자체 cmake CLI). TASK-033 코드 완료(Doctor+listProjects+chips+File API, F5 대기). 남음: TASK-034/035 |
