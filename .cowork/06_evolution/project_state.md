...
| 최신 배포 | **v0.5.0** (`devswitcher-tools-0.5.0.vsix`, 태그 `v0.5.0` origin push) — 다음 v0.6.0(Node/TS) |
...
- **다음**: **MS-016 Node/TS 어댑터(v0.6.0) 착수**(6번째 언어). v0.5.0(Go) main 병합·`v0.5.0` 태그·origin push 완료. 별도 트랙: TC-11(WSL, Deferred).
...
1. **MS-016 — Node/TS 어댑터 (v0.6.0) 착수**: 착수 시 Task 분해 — 감지(`package.json`)·npm/pnpm scripts·`tsc`·node `--inspect` 디버그. 6번째 언어(완료 시 6개 언어 완성). ※ v0.5.0(Go)은 main 병합·태그·push 완료.
...
| Release | v0.5.0 | `devswitcher-tools-0.5.0.vsix` | Done | **Go 어댑터(MS-015)** — 감지·target 칩·`go build`/`go run`·delve 디버그·Doctor·F20 생성. 13파일 237.64KB. 설치 스모크 통과(`@0.5.0`). F5 통과·unit 219. main FF·`v0.5.0` 태그·origin push·브랜치 삭제 |
