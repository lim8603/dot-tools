...
| 활성 Task | TASK-041 (CMakePresets.json 지원 — 신규 Planned). TASK-033·034·035 완료·커밋(035 F5 Run+Debug 통과) |
| 상태 | Green |
| 대화 언어 | 한국어 |
| 작업 문서 언어 | 한국어 |
| 공식 산출물 문서 언어 | 한국어 |
...
- **다음**: **TASK-041**(CMakePresets.json: Preset 칩+`--preset`, 프리셋 있으면 profile/arch 대체·없으면 폴백) → MS-012 F5·병합. 이후 C-6 Run Group(MS-013, TASK-036~040).
...
| TASK-041 | CMakePresets.json 지원 | AI | Planned | 2026-08-16 | 신규(세션 #009). Preset 칩(`configurePresets` 읽기)+`cmake --preset` configure. 프리셋 있으면 profile/arch 대체·없으면 현행 폴백·target 유지. 실 프로젝트 GCC/Clang/MSVC 전환 충족. (037~040=MS-013 예약이라 041 채번) → MS-012 F5·병합 |
...
2. **TASK-041 (신규 세션)** — **CMakePresets.json 지원**: `CMakePresets.json`(+User) `configurePresets` 읽어 **Preset 칩** → `cmake --preset <name>` configure. 프리셋 있으면 profile/architecture 칩 대체(프리셋이 컴파일러+제너레이터+빌드타입 인코딩)·없으면 현행 `-S -B -D` 폴백. target 칩·디버거 자동판별은 그대로 재사용. → MS-012 F5 → main 병합.
...
| Milestone | MS-012 | C++ (CMake) 어댑터 실구현 | In Progress | C-7 3/3. ADR-014. TASK-033·034·035 F5통과·커밋(감지+chips+File API·configure/build 주입·run/debug 자동판별). **핵심 루프 완성.** 남음: TASK-041(CMakePresets)→F5→병합 |
