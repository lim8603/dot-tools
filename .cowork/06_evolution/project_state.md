...
| 협업 실행 모드 | solo |
| 현재 Phase | Build (MS-001~012 Done · v0.3.0 / **MS-013 Run Group(C-6) In Progress**) |
| 활성 Intent | INT-001 (Approved — 완료 조건 = C-7 다언어 실구현[Done] + C-6 Run Group[진행]) |
| 활성 Milestone | **MS-013 Run Group (C-6)** — In Progress. TASK-036 Review. 완주=**v0.4.0**(v1.0.0 아님, D-19) |
| 활성 Task | **TASK-040**(통합테스트·검증·문서 → v0.4.0). TASK-036·037·038 **F5 통과·Done**(미커밋) |
| 상태 | Green |
| 대화 언어 | 한국어 |
| 작업 문서 언어 | 한국어 |
| 공식 산출물 문서 언어 | 한국어 |
...
- **MS-001~012 Done** (전부 main 병합·v0.3.0). INT-001 완주 조건 = C-7(Done)+C-6(진행). **MS-013 Run Group 착수** — TASK-036(그룹 모델·저장·순수 계획/검증·ADR-015) 코드 완료·unit 186. **버전 정책 반전(D-19)**: MS-013 완주=**v0.4.0**, v1.0.0은 추가 기능·논의(MS-014/TASK-042) 후 **Human 명시 선언** 시. **C-3 폐기**(D-15).

### 현재 작업 스트림
> 핵심 작업 스트림만 3~5줄 이내로 유지한다.

- **진행(세션 #011)**: **MS-013 Run Group — TASK-036·037·038 완료(Review)**. Human 결정=준비 **프로세스 시작 감지**·**Run 전용**·**v0.x.x 유지**(D-19). 036=모델·저장·순수 `runGroupPlan`·**ADR-015**. 037=`TaskRunner.start`·순수 `groupSequencer`·`GroupOrchestrator`·커맨드. 038=설정 페이지 **Run Groups 탭**(그룹 CRUD·멤버·종속 편집·Run/Stop)·상태바 **그룹 런처**(`setGroups`)·순수 편집 헬퍼(`withMember`/`withMemberDependencies`). unit **196**(+24)·esbuild prod 99.3kb. **미커밋**(main).
- **다음**: **전체 F5**(그룹 정의→계층 기동/정리, auth→api→web) → **TASK-040**(통합테스트·검증·문서 → **v0.4.0**). 039(준비감지)=후속 마이너. **TASK-042**=v1.0.0 완주 결정 게이트(D-19).
...
|---------|------|------|------|---------------|-----------|
| TASK-038 | 그룹 정의/트리거 UI | AI | Done | 2026-08-17 | **F5 통과**(스테이지 순서 UI·skip·통합 메뉴·상태바 아이콘, D-20). unit 203. 미커밋 |
| TASK-040 | Run Group 통합 테스트 + 검증 + 문서 | AI | Planned | 2026-08-17 | 통합 테스트 + test_case/verification + README/CHANGELOG → **v0.4.0**(C-6 충족). 커밋 필요 |
| TASK-042 | 추가 기능·논의 + v1.0.0 완주 결정 | Human | Planned | 2026-08-17 | MS-013(v0.4.0) 후 게이트 — 남은 기능·논의 수집 → v1.0.0 완주 여부 Human 결정(D-19) |

> **TASK-001~038·041 Done(MS-001~012 + MS-013 036·037·038 F5 통과, unit 203).** MS-013 엔진+UI 완료 → **TASK-040**(통합·문서→**v0.4.0**). 039(준비감지)=후속 마이너. Done된 CMake Task(033~035·041) 상세는 session_2026-08-17_010 참조. C-3 폐기(D-15). **미커밋**(main). 수동검증 TC-11(WSL) Deferred.
...
1. **전체 F5 (MS-013)** — 설정(`DevSwitcher: Open Settings`) → **Run Groups 탭** → 그룹 생성·멤버 체크·종속(starts after) 설정 → **Run**(또는 상태바 `$(run-all) Groups`) → 계층 순서로 태스크 기동(프로세스 뜨면 다음)·**Stop**으로 일괄 종료. 다언어 조합(예: Rust api ← Python web) 권장.
2. **TASK-040** — F5 통과 후 통합 테스트(커맨드/스모크)·test_case/verification·README/CHANGELOG → **v0.4.0** 배포(C-6 충족).
3. (선택) 커밋 — 소스 `feat(core): run group (TASK-036~038)`, 문서 `docs(cowork)`. 브랜치 `feature/ms-013-run-group` 권장(현재 main·미커밋). 이후 **TASK-042**(v1.0.0 결정, D-19). 039(준비감지)=후속 마이너. 별도 트랙: TC-11(WSL, Deferred, GAP-001).
...
| Milestone | MS-013 | Run Group (C-6) | In Progress | 세션 #011. ADR-015·D-20. **TASK-036·037·038 F5 통과**(모델·엔진·설정 페이지 스테이지 UI·상태바 통합 메뉴, unit 203). 남음: **TASK-040**(통합·문서). 완주=**v0.4.0**(D-19) |
