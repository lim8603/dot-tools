...
| 협업 실행 모드 | solo |
| 현재 Phase | Build (MS-001~015 Done · **v0.5.0 배포** / **v1.0.0 로드맵 D-21** — MS-016~018 → MS-014; MS-019/020 INT-002) |
| 활성 Intent | INT-001 (Approved — 등록 조건 C-7+C-6 충족. v1.0.0은 로드맵 D-21 완주 후 Human 선언, D-19) |
| 활성 Milestone | **없음** — MS-015 Done·v0.5.0. 다음: **MS-016 Node/TS(v0.6.0)** (v1.0.0 로드맵 D-21) |
| 활성 Task | **없음** — 다음: **MS-016 착수 시 Task 분해**. TASK-042(v1.0.0 완주 결정)는 로드맵(MS-018) 완료 후 |
| 상태 | Green |
| 대화 언어 | 한국어 |
| 작업 문서 언어 | 한국어 |
| 공식 산출물 문서 언어 | 한국어 |
...
| 최신 배포 | **v0.5.0** (`devswitcher-tools-0.5.0.vsix`, Go/MS-015 — vsix·스모크 완료) — 다음 v0.6.0(Node/TS) |
...
- **MS-001~015 Done · v0.5.0 배포**(Go 어댑터, 세션 #012). INT-001 등록 조건 C-7+C-6 충족. **v1.0.0 로드맵(D-21)**: MS-015 Go(v0.5.0 ✅) → MS-016 Node/TS(v0.6.0) → MS-017 단축키(v0.7.0) → MS-018 준비감지(v0.8.0) → MS-014 v1.0.0 최종점검+게시(Human 선언, D-19). **원격디버그(MS-019)·크로스컴파일(MS-020)은 INT-002**(D-22). **다음: MS-016(Node/TS) 착수.** **C-3 폐기**(D-15).
...
- **완료(세션 #012, 계속)**: **MS-015 Go 어댑터 → v0.5.0 배포.** TASK-043(감지·target 칩·createProject·Doctor)·044(build/run·`assembleGoArgs`·`$devswitcher-go` 매처·옵션 카탈로그)·045(delve 디버그·`buildDelveConfig`). **F5 전부 통과**(Doctor 슬라이스→build/run→delve 중단점·`dlv` 자동설치). unit **219**·실 go1.26.6 스모크·`devswitcher-tools-0.5.0.vsix`(13파일 237.64KB) 스모크. 브랜치 `feature/ms-015-go-adapter` 커밋.
- **다음**: **MS-016 Node/TS 어댑터(v0.6.0) 착수**(6번째 언어). 선행: v0.5.0 main 병합·태그·push(Human 확인 후). 별도 트랙: TC-11(WSL, Deferred).
...
|---------|------|------|------|---------------|-----------|
| MS-016 | Node/TS 어댑터 (v0.6.0) — 로드맵 2단계 | AI | Planned | 2026-08-17 | **다음 착수** — 착수 시 Task 분해(감지 `package.json`·npm/pnpm scripts·`tsc`·node `--inspect`). 6번째 언어 |
...
> **TASK-001~045 Done(039 제외)·MS-015 Go 완료·v0.5.0 배포**(세션 #012, unit **219**). **v1.0.0 로드맵(D-21)**: MS-015 Go(✅) → MS-016 Node/TS → MS-017 단축키 → MS-018 준비감지 → MS-014 최종점검+게시. **원격디버그(019)·크로스(020)는 INT-002**(D-22). **다음: MS-016 착수**(Task 미분해). Go Task(043~045) 상세는 session #012, Run Group(036~040)은 #011. C-3 폐기(D-15). TC-11(WSL) Deferred.
...
0. (선행, Human 확인 후) **v0.5.0 main FF 병합 + `v0.5.0` 태그 + origin push** — 현재 `feature/ms-015-go-adapter`에 커밋됨(TASK-043~045+release).
1. **MS-016 — Node/TS 어댑터 (v0.6.0) 착수**: 착수 시 Task 분해 — 감지(`package.json`)·npm/pnpm scripts·`tsc`·node `--inspect` 디버그. 6번째 언어(완료 시 6개 언어 완성).
2. 이후 로드맵 순차: MS-017 단축키(v0.7.0) → MS-018 준비감지(v0.8.0) → **MS-014 v1.0.0 최종점검+Marketplace 게시+GitHub Release**(Human 완주 선언). 원격디버그(MS-019)·크로스컴파일(MS-020)은 INT-002. 별도 트랙: TC-11(WSL, Deferred, GAP-001).
...
| Release | v0.5.0 | `devswitcher-tools-0.5.0.vsix` | Done | **Go 어댑터(MS-015)** — 감지·target 칩·`go build`/`go run`·delve 디버그·Doctor·F20 생성. 13파일 237.64KB. 설치 스모크 통과(`@0.5.0`). F5 통과·unit 219. (main 병합·`v0.5.0` 태그·push는 Human 확인 후) |
