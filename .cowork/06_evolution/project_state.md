...
| 활성 Task | TASK-036 (CMakePresets.json 지원 — 신규 Planned). TASK-033·034·035 완료(035 F5 Run+Debug 통과, 커밋 대기) |
| 상태 | Green |
| 대화 언어 | 한국어 |
| 작업 문서 언어 | 한국어 |
| 공식 산출물 문서 언어 | 한국어 |
...
- **MS-001~011 Done** (병합·push). INT-001 완주 로드맵 = C-7(다언어 실구현)+C-6(Run Group). **현재: MS-012 CMake(C-7 3/3)** — ADR-014. **TASK-033 F5·커밋**(감지+chips+File API), **TASK-034 F5·커밋**(prepareInvocation+configure/build 주입+resolveExecutable), **TASK-035 F5통과·커밋대기**(run build-then-exec + debug 컴파일러 자동판별[cppvsdbg/cppdbg+gdb/lldb]+override). **CMake 핵심 루프(switch/build/run/debug) 완성.** 남음: **TASK-036**(CMakePresets.json — 실 프로젝트 GCC/Clang/MSVC 전환). **C-3 폐기**(D-15).

### 현재 작업 스트림
> 핵심 작업 스트림만 3~5줄 이내로 유지한다.

- **완료**: TASK-033·034 F5·커밋(`8664be9`·`4ca0330`·`c0f564a`·`a231e24`·`814d654`). **TASK-035 F5통과(Run+Debug)** — 컴파일러 자동판별 디버거+override, run build-then-exec. unit **165**·실 cmake 스모크 통과. 커밋 대기.
- **다음**: TASK-035 커밋 → **TASK-036**(CMakePresets.json: Preset 칩+`--preset`, 프리셋 있으면 profile/arch 대체·없으면 폴백) → MS-012 F5·병합. 이후 C-6 Run Group(MS-013).
...
|---------|------|------|------|---------------|-----------|
| TASK-033 | CMakeBridge + listProjects + chips | AI | Done | 2026-08-16 | F5·커밋(`8664be9`+`4ca0330`). Doctor+listProjects+chips+File API |
| TASK-034 | configure/build 주입 + resolveExecutable | AI | Done | 2026-08-16 | F5·커밋(`c0f564a`+`a231e24`+`814d654`). prepareInvocation 훅+`cmake --build`+`-D` 주입+resolveExecutable |
| TASK-035 | run + debug (컴파일러 자동판별 디버거) | AI | Review | 2026-08-16 | **F5 통과(Run+Debug)·커밋 대기** — run build-then-exec(`runRequiresBuild`·`peekArtifact`) + debug(File API `toolchains`→`debuggerFor` MSVC/GNU/Clang→cppvsdbg/cppdbg+gdb/lldb + `devSwitcher.cmake.debugger` override + 동적 확장 ensure). unit 165·실 cmake 스모크 |
| TASK-036 | CMakePresets.json 지원 | AI | Planned | 2026-08-16 | 신규(세션 #009). Preset 칩(`configurePresets` 읽기)+`cmake --preset` configure. 프리셋 있으면 profile/arch 대체·없으면 현행 폴백·target 유지. 실 프로젝트 GCC/Clang/MSVC 전환 충족. 이후 MS-012 F5·병합 |
...
1. **TASK-035 커밋**(feat+docs) — 브랜치 `feature/ms-012-cmake-adapter`(미병합). ※ 세션 #009 마무리에서 커밋함.
2. **TASK-036 (신규 세션)** — **CMakePresets.json 지원**: `CMakePresets.json`(+User) `configurePresets` 읽어 **Preset 칩** → `cmake --preset <name>` configure. 프리셋 있으면 profile/architecture 칩 대체(프리셋이 컴파일러+제너레이터+빌드타입 인코딩)·없으면 현행 `-S -B -D` 폴백. target 칩·디버거 자동판별은 그대로 재사용. → MS-012 F5 → main 병합.
...
| Milestone | MS-012 | C++ (CMake) 어댑터 실구현 | In Progress | C-7 3/3. ADR-014. TASK-033·034·035 F5통과·커밋(감지+chips+File API·configure/build 주입·run/debug 자동판별). **핵심 루프 완성.** 남음: TASK-036(CMakePresets)→F5→병합 |
