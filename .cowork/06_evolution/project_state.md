...
| TASK-035 | run + debug (컴파일러 자동판별 디버거) | AI | Done | 2026-08-16 | **F5 통과·커밋**(`102c150`+`61f6ac3`). run build-then-exec + debug 컴파일러 자동판별+override |
| TASK-041 | CMakePresets.json 지원 (Preset 칩) | AI | Done | 2026-08-17 | **F5 통과(세션 #010)** — `ChipDescriptor.appliesTo` 동적 대체(Preset 칩↔profile/arch) + `parseConfigurePresets`(inherits/hidden/user)·`resolvePresetBinaryDir`(매크로) + `cmake --preset` configure(binaryDir·--config 생략) + target/디버거 재사용. unit **172**·실 프리셋 스모크·**실 F5**(msvc-x64 build/run/debug). 커밋(feat+docs)·미병합. → **MS-012 main 병합** |
