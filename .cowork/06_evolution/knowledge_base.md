...
| 3 | MSBuild는 솔루션 빌드와 단독 vcxproj 빌드의 출력 경로가 다르다 — 멤버 프로젝트엔 `/p:SolutionDir` 주입 | 없음(유지보수) | MS-022 | 기본 OutDir이 `$(SolutionDir)$(Platform)\$(Configuration)\`이라 **솔루션으로 빌드하면 솔루션 루트에**, `.vcxproj`를 단독 빌드/평가하면 `$(SolutionDir)`가 미정의(빈 값)라 **프로젝트 로컬 `x64\Debug\`에** 출력된다(실측: demo.sln 빌드→`demo\x64\Debug\app.exe`, 단독 `-getProperty:TargetPath`→`app\x64\Debug\app.exe` 불일치). VS IDE는 항상 SolutionDir를 넘기므로 **멤버 vcxproj를 CLI로 빌드·평가할 땐 `/p:SolutionDir=<구분자로 끝나는 솔루션 디렉토리>`를 명시 주입**해 빌드 출력과 경로 해석을 일치시킨다(트레일링 구분자 필수 — MSBuild 관례상 `$(SolutionDir)`는 구분자 포함). 부수: PlatformToolset 미지정 vcxproj는 v100으로 폴백해 MSB8020이 나므로 픽스처엔 `$(DefaultPlatformToolset)`을 명시하면 어떤 VS 버전에서도 빌드된다 | 세션 #017, MS-022 TASK-063 실 스모크 | 2026-08-19 |
...
| 11 | 정보 토스트(`showInformationMessage`)는 한 줄 렌더 — 긴 사유 메시지는 짤린다 | validateAction 차단 사유 등 토스트로 전달하는 안내 문구 | 없음(유지보수) | MS-022 | VSCode 정보 토스트는 줄바꿈 없이 한 줄로 렌더되어 긴 문장은 끝이 잘린다(F5 실증: VS lib 차단 안내). **토스트 문구는 핵심 사실+행동 하나로 단문화**("'X' is a static library — build only." 수준). 상세 설명이 필요하면 토스트가 아니라 Doctor/설정 페이지/문서로 보낸다. 접두사("DevSwitcher: ")도 폭을 차지하므로 감안 | 세션 #017, MS-022 F5 (Human 피드백) | 2026-08-19 |
