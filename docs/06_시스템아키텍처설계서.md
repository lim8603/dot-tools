...
DevSwitcher Tools는 **여러 언어(Rust·C++/CMake·C++/Visual Studio·C#·Python·Go·Node.js/TypeScript)의 프로젝트를 하나의 상태바 UX로 전환·빌드·실행·디버그하는 VSCode 확장**이다. 이 프로젝트의 "도메인"은 다언어 빌드/디버그 오케스트레이션 자체이며, 확장 호스트 프로세스 하나가 단일 Bounded Context다(ASM-001).
...
> 출처: `domain_model.md` §도입·§7, ADR-003·007·011·013·001·002·015·018. 6개 언어 구성은 프로젝트 컨텍스트(v1.0.0) 및 ADR-014(CMake)·ADR-016(Node)으로, 7번째 어댑터(vs, v1.2.0)는 ADR-021로 확인.
...
| ADR-019 | CMake 프로젝트 계층 = `project()` 루트는 최상위, 타깃 선언 디렉토리는 하위 + `validateAction` 훅 | 2026-08-18 | ADR-014 후속 |
| ADR-020 | Run Group 멤버를 디버그로도 기동 | 2026-08-18 | ADR-015 후속 |
| ADR-021 | Visual Studio 어댑터 = MSBuild 직접 구동 (Windows 전용, `.vcxproj`만) | 2026-08-19 | ADR-014 정신 |
...
#### A-2. 부작용 없는 질의와 정리 — v1.2.1 · v1.3.0 【v1.3.0 추가】

ADR 번호가 붙지 않은 결정이지만 아키텍처 불변식에 해당하므로 여기 기록한다.

- **"선택은 빌드가 아니다" (v1.2.1).** 칩 목록을 얻는 일이 파일을 만들어서는 안 된다. CMake는 타깃 목록을 File API로 읽는데 그것이 **구성된 빌드 트리를 요구**하고, 목록을 그리려던 6개 경로(활성화·재스캔·전환·설정 페이지 열기·카드 클릭·카드 기본값)가 전부 `cmake -S . -B <project>/build`까지 내려갔다. 사용자가 읽기 전용으로 다루는 서브모듈에 우리 파일이 로컬 변경으로 나타났다.
  - **해법은 UI가 아니라 계약에 두었다.** `ChipDescriptor.listItems`/`defaultValue`에 `opts.probe`를 추가해 "부작용 없이 아는 것만 답하라"를 표현한다. UI는 여전히 언어를 모르고(INV-2), 무엇이 비싼지는 어댑터가 판단한다.
  - **파급 2건을 함께 막았다**: ① 빈 목록을 "유효 항목 0개"로 읽으면 정합성 패스가 저장된 선택을 삭제한다 → 빈 답은 "답 없음"으로 처리 ② 기본값 시딩을 전면 금지하면 Run Group 멤버가 미빌드 CMake일 때 그룹이 막힌다 → 전환=probe off, 액션 직전=probe on으로 분리.
  - v1.3.0에서 이 계약을 **전 어댑터로 확대**했다. 다만 cmake 외의 어댑터는 파일을 만들지 않았음이 실증되었으므로(빈 사본에서 명령 실행) 그쪽 수정은 결함 제거가 아니라 비용·정합성 개선이다.
- **"정리는 구성을 유발하지 않는다" (v1.3.0).** Clean/Delete Build Tree 경로는 `prepareInvocation`(CMake의 configure 진입점)을 호출하지 않는다. 정리 요청이 빌드 트리를 만들어내면 위 결함이 새 문으로 그대로 돌아온다. 미구성 트리는 "정리할 것 없음"으로 답한다.
- **삭제 권한은 어댑터가 아니라 코어가 정한다 (v1.3.0).** 확장에서 유일하게 파괴적인 동작이므로, 어댑터의 `buildTreeDirs`는 **후보**만 제안하고 순수 모듈 `core/cleanPlan.ts`가 워크스페이스 밖·워크스페이스 루트·**프로젝트 소스 디렉토리**를 거부한다. 인-소스 구성이나 오버레이 오설정이 삭제를 소스 트리로 겨누는 것을 막기 위해, 판단 로직을 `fs.delete` 호출부가 아니라 **전수 테스트 가능한 순수 코드**에 두었다.
...
- **ADR-021** 【v1.2.0】: Visual Studio 어댑터는 MSBuild를 직접 구동한다(ADR-014가 CMake에 세운 "확장 의존 없이 CLI를 직접 부른다"의 연장). 결정 3건: ① 빌드 디렉토리의 CMake 생성 `.sln`/`.vcxproj`는 `CMakeCache.txt` 마커로 제외 — 안 그러면 CMake 프로젝트 하나가 스위처에 두 번 뜬다 ② `.csproj`는 dotnet 어댑터 소유로 두고 vs는 `.vcxproj`만 가져간다 ③ Windows 전용. 실행 경로는 추측하지 않고 `-getProperty:TargetPath`로 평가한다(DD-05). **솔루션 빌드와 멤버 단독 빌드의 출력 경로가 갈라지는 문제**는 멤버 빌드에 `/p:SolutionDir` 주입으로 해결했다(실측 실증).
