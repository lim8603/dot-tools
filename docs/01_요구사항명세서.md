...
DevSwitcher Tools는 여러 언어 프로젝트(Rust · C++/CMake · C# · Python · Go · Node.js/TypeScript)를 하나의 VSCode 상태바 UX로 전환·빌드·실행·디버그하는 VSCode 확장이다. 본 문서는 프로젝트 Intent(INT-001)를 구체적 요구사항으로 분해한 명세로, 기능적·비기능적 요구사항, 제약, 의존성, 가정, 미확정 사항을 정의한다.

- **관련 Intent:** INT-001
- **기준 문서 버전:** v1 (설계서 v1.1 반입, 상태: Approved)
- **상세 동작 기준:** 기능별 상세 동작은 `functional_spec.md`(F1~F19)를 기준으로 하며, 본 문서는 요구 수준으로만 정리한다.

> **범위 주석 (v1.0.0 시점):** 기준 문서 일부는 초기 계획("v1 실구현은 Rust 단독, 나머지는 스텁") 기준으로 서술되어 있다. v1.0.0 릴리즈(2026-08-17) 현재는 **6개 언어(Rust · C++/CMake · C# · Python · Go · Node.js/TypeScript) 어댑터가 전부 구현 완료**된 상태이며, 해당 차이가 있는 항목에는 개별 주석을 달았다. 기준 문서 원문 표현은 왜곡 없이 유지한다.
